...
class LookupGenerator(object):
    ...
    Currently supported: (2, simplex), (3, simplex), (2, cube), 
      (3, cube), (4, cube)
    """
    def __init__(self, dim, basic_type, verbose=False):
        print "creating ", (dim, basic_type)
        self.dim = dim
        ...
        self.transformation_group = set([Transformation(1, 0, 
                                                        range(self.p_size))
                      ,Transformation(1,1,range(self.p_size))])
        i = 0
        g_size = 0
    ...
    def print_base(self, prefix="lut", face_count=0, cell_count=0):
        ...
        print "# base cases %s %iD:" % (self.basic_type, self.dim)
        i = 0
        for entry in self.base_cases:
            case = list(entry.case)
            case.reverse()
            print "# " + ",".join(str(x) for x in entry.case) \
                + " -> " + "".join(str(x) for x in case)
            while len(entry.faces) < face_count:
                entry.faces.append([])
            print prefix + ".base_cases[%i].faces = " % i + repr(entry.faces)
            while len(entry.exterior) < cell_count:
                entry.exterior.append([])
            print prefix + ".base_cases[%i].exterior = " \
                % i + repr(entry.exterior)
            while len(entry.interior) < cell_count:
                entry.interior.append([])
            print prefix + ".base_cases[%i].interior = " \
                % i + repr(entry.interior)
            i += 1
        print
